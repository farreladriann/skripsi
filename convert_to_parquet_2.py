...
def main():
    ...
    for csv_rel_path in gps_csv_files:
        ...
        for chunk in pd.read_csv(
            ...
            # Konversi tipe data
            chunk["latitude"] = pd.to_numeric(chunk["latitude"], errors="coerce").astype(
                "float32"
            )
            chunk["longitude"] = pd.to_numeric(chunk["longitude"], errors="coerce").astype(
                "float32"
            )
            chunk["timestamp"] = pd.to_numeric(chunk["timestamp"], errors="coerce").astype(
                "Int64"
            )
        ...
        print(
            f"    → {file_rows:,} baris | CSV: {csv_size / 1e6:.0f} MB → Parquet: {pq_size / 1e6:.0f} MB ({ratio:.1f}x kompresi) | {elapsed:.1f}s"
        )
    ...
    if os.path.exists(pg_out):
        ...
        print(
            f"    → {len(df_pg):,} baris | CSV: {pg_csv_size / 1e6:.0f} MB → Parquet: {pg_pq_size / 1e6:.0f} MB ({ratio:.1f}x kompresi) | {elapsed:.1f}s"
        )
